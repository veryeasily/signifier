...
var Deleter, LOGGING, SOCKET_ADDRESS, Sign, Signifier, SignifierHelpers, Walkthrough;
...
Signifier = (function() {
  ...
  Signifier.getNeighborhood = function() {
    ...
    this.socket.on('heresYourHood', function(links) {
      if (LOGGING) {
        console.log("response from heresYourHood!");
        console.log(links);
      }
      return Signifier.queWorkingLinks(links);
    });
    return observer = new WebKitMutationObserver(function(mutations) {
      var possibleElts, testNodeForSiggg;

      possibleElts = [];
      testNodeForSiggg = function(node) {
        var mutation, ref, _i, _j, _k, _len, _len1, _len2, _ref, _results;
  ...
  Signifier.queWorkingLinks = function(links, parent) {
    ...
    Signifier.fillInSign = function(link, parent) {
      ...
      actual = possibles.filter(function(ind) {
